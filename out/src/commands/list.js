...
function listProblems() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const useEndpointTranslation = settingUtils.shouldUseEndpointTranslation();  // 是否进行翻译
            const result = yield leetCodeExecutor_1.leetCodeExecutor.listProblems(showLocked, useEndpointTranslation); // 根据翻译选项获取结果
            const result_en = yield leetCodeExecutor_1.leetCodeExecutor.listProblems(showLocked, false);  // 获取未翻译结果
            const problems = [];
            const lines = result.split("\n"); 
            const lines_en = result_en.split("\n"); // 对未翻译结果进行分割
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                const line_en = lines_en[i];
                const match = line.match(reg);
                const match_en = line_en.match(reg);
